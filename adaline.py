...
def entrenamiento ():
   #Adaline harlim
    ...
    F_aprendizaje = float(input("Digite el valor  del factor de aprendizaje (α) : "))
    ...
    while total_errores != 4: #Funcion que determina el numero de iteraciones
        ...
        for i in arreglo:
            ...
            errores.append(e1)
            if (e1 != 0):
                w = (w+((2)*(F_aprendizaje)*e1*np.transpose(np.array(patron))))
                b = (b+(e1*(F_aprendizaje)*(2)))
...
opcion = int(input("Digite su opcion a elegir;  {1.- Adaline en hardlim}       {2.- Adaline en harlimsimetrico} "))
